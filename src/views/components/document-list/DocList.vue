    ...
    <template v-else>
                ...
                <ion-row>
                    ...
                    <ion-col :size="2" v-if="data.length > 0" class="ion-hide-sm-down">
                        <div style="display: flex; justify-content: right;">
                            ...
                            <ion-button @click="onReloadData" fill="clear" shape="round"
                                v-tooltip.left="`${t('Refresh Data')}`">
        ...
        <slot v-if="data && data.length > 0" :item="data">
                            ...
                            <slot :name="col.fieldname" :item="slotProps.data" :index="col.fieldname + '_' + index">
                                ...
                                <template v-else-if="col.url">
                                    <template v-if="col.id_field">
                                        <router-link v-if="slotProps.data[col.id_field]"
                                    ...
                                    <template v-else>
                                        <router-link v-if="slotProps.data[col.fieldname]"
            ...
            <ComDocCard v-else :data="d" v-for="(d, index) in data" :key="index" :fields="options.columns"
                :titleFields="options.title_fields || meta.title_field"
                :subTitleFields="options.subTitleFields || meta.search_fields" />

        </slot>
        <div v-else>
            <slot name="empty">

                <ComNoRecord :title="t('No Record Found')" :message="emptyRecordMessage"
                    :actions="options.noRecordActions" />
            </slot>
        </div>
