<template>
 ...
 <div v-if="options">
  <template v-for="(d,index) in options" :key="index">
    <ComSelect v-if="d.ft=='Link'" 
      :docType="(typeof d.options) == 'string'?d.options:''"
      :options="(typeof d.options) != 'string'?d.options:''"
     v-model:value="filter[d.f]" :multiple="d.multi==1" @onSelected="onFilterReport" :clear="d.clear==1" @onClear="onClearSelect(d)"
      :filters="getSelectDefaultFilter(d)"
      :modalType="d.modalType"
      :label="d.label"

    />
    <ComSelectDateFilter v-else-if="d.ft=='Date'"  @onSelect="onSelectDate($event, d)" :defaultTimespan="d.default" :clear="d.clear==1" :showOperator="d.show_op==1"/>
  </template>
 </div>
