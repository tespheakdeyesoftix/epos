<template>
    <ion-card v-for="(p, index) in doc.product_price" :key="index">
      ...
      <ion-card-title>{{ t("Product Price:") }} {{ index +1 }}</ion-card-title>
        ...
        <ion-card-content>
            <Stack gap="10px">
                <com-input label="Barcode" placeholder="Product Code" v-model="p.barcode"
                type="BarcodeScanerInput" 
                label-placement="floating" 
                fill="outline" />
                <!-- price rule -->
                <ion-label>{{ t("Price Rule") }}
                    ...
                    <ion-input type="number" :label="t('Conversion Factor')" :placeholder="t('Conversion Factor')"
                        v-model="p.conversion_factor" 
                        label-placement="floating" 
                        fill="outline" 
                        ></ion-input>
                        <!-- price -->
                        <ion-input type="number" :label="t('Price')" :placeholder="t('Price')"
                        v-model="p.price"
                        label-placement="floating" 
                        fill="outline" 
                        ></ion-input>
        ...
        <ion-button shape="round" class="ion-margin" color="danger" fill="solid" @click="DeletePrice(index)">{{ t("Delete Price") }}</ion-button>
    </ion-card>
