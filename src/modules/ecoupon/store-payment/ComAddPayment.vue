<template>
  ...
  >
    <Message
      v-if="doc.name && doc.docstatus == 0 && !docChanged"
      severity="info"
      class="mb-3"
    >
      {{ t('Submit this document to confirm') }}
    </Message>
    <Message v-if="doc.pos_profile && !isNaN(creditBalance)" severity="success" class="mb-3">
      {{ t('Current Balance:') }} <ComCurrency :value="creditBalance" />
    </Message>
    ...
    <stack>
      <stack row equal>
        <ComSelectInput
          docType="POS Profile"
          v-model="doc.pos_profile"
          :label="t('Select Store')"
          @onSelected="onSelectPOSProfile"
        />
...
async function onSelectPOSProfile(profile) {
  const res = await app.getApi(
    'epos_restaurant_2023.selling.doctype.store_payment.store_payment.get_vendor_credit_balance',
    {
      pos_profile: profile.name
    }
  )
  if (res.data !== undefined && !isNaN(Number(res.data))) {
    creditBalance.value = Number(res.data)
  } else {
...
async function onConfirmClick() {
  // Redirect if no open shift
  if (!app.setting?.cashier_shift?.name) {
    await modalController.dismiss()
    router.push('/message/No Shift Opened?&return_url=/store-payment-list')
    return
  }
...
onMounted(async () => {
  ...
  // Redirect if no open shift on mount
  if (!app.setting?.cashier_shift?.name) {
    await modalController.dismiss()
    router.push('/message/No Shift Opened?&return_url=/store-payment-list')
    return
