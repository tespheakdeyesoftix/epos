  ...
  <ion-card class="relative">
    ...
    <ion-card-content class="ion-no-padding">
      ...
      <ion-grid class="ion-no-padding">
        <ion-row>
          <ion-col v-for="(p, index) in products" :key="index" size="6" size-md="4">
            <ion-card :disabled="!topUpCouponInfo" @click="onSelectCoupon(p)" class="coupon-card ion-no-padding">
              <ion-card-content class="content-center ion-no-padding">
                ...
                <ion-card :color="p.name === topUpSaleProduct?.product_code ? 'success' : 'light'"
                  class="top-up-product-name">
...
async function onSelectCoupon(p) {
  ...
  if (p.is_open_product == 1) {
    const result = await app.openModal({
      ...
      cssClass: app.utils.isMobile()? "full" : "top-up-coupon-amount"
    })
    if (!result) return
